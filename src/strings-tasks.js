...
function findLongestWord(sentence) {
  const anyWord = sentence.split(' ');
  const longestWord = anyWord.reduce((longest, current) => {
...
function reverseWords(str) {
  ...
  const reverseEachWord = (word) => word.split('').reverse().join('');
  const reversedWords = eachWord.map(reverseEachWord);
